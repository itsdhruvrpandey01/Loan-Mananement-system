...
public class UserServiceImpl implements UserService{
    ...
    public ApplicationResponse applyLoan(ApplicationDto applicationDto,UUID customerId) {
        LoanScheme loanScheme = mapper.map(loanSchemeService.findLoandById(applicationDto.getLoanSchemeId()),LoanScheme.class);
        ...
        if(applicationDto.getApplicantAge()>loanScheme.getMaxAge() || applicationDto.getApplicantAge()<loanScheme.getMinAge()) {
        	throw new LoanELigibiltyException("Applicant age must be between "+loanScheme.getMaxAge() +" - "+loanScheme.getMinAge());
        }
        if(applicationDto.getLoanAmount()>loanScheme.getMaxLoanAmount() || applicationDto.getLoanAmount()<loanScheme.getMinLoanAmount()) {
        	throw new LoanELigibiltyException("Requested loan amount must be between "+loanScheme.getMaxLoanAmount()+" "+loanScheme.getMinLoanAmount());
        }
        if(applicationDto.getMonthlyIncome()<loanScheme.getMinIncome()) {
        	throw new LoanELigibiltyException("Applicants Income is lesser than required income "+loanScheme.getMinIncome());
        }
        Double totalLoanAmount = loanCalculator.calculateTotalAmount(applicationDto.getLoanAmount(), loanScheme.getInterestRate(), applicationDto.getTenure());
...
//        application.setDocuments(docs);
	...
	public User findUserById(UUID userId) {
		return this.userRepository.findById(userId).orElseThrow(() -> new UserNotFoundException("No customer with id " + userId));
	}


	@Override
	public DocumentResponseDto uploadDocuments(UUID applicationId,List<MultipartFile> files, List<String> docTypes) throws IOException {
		Application application = applicationRepository.findById(applicationId).orElseThrow(()->new UserNotFoundException("No Application with id " + applicationId));
	...
	public UserDetailsResponseDto findUserProfileById(UUID userId) {
		...
		if(user.getProfilePicture()!=null)
			userDetail.setProfilePictureResponseDto(mapper.map(user.getProfilePicture(), ProfilePictureResponseDto.class));
		return userDetail;
	}
